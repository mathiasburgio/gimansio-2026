...
class SimpleCRUD{
    ...
    clearFields(){
        ...
        $(this.padre).find("[type='checkbox'], [type='radio']").prop("checked", false);
    ...
    getDataToSave(){
        ...
        $(this.padre).find("[crud='fields'] [name]").each((index,ev)=>{
            ...
            if($(ev).attr("type") == "checkbox" || $(ev).attr("type") == "radio") val = $(ev).prop("checked");
        ...
        for(let prop in element){
            ...
            if(dx.length == 1){
                if(dx.attr("type") == "date"){
                    if(val && fx && typeof fx.toInputDate == "function"){
                        dx.val(fechas.toInputDate(val));
                    }
                }else if(dx.attr("type") == "datetime-local"){
                    if(val && fx && typeof fx.toInputDatetime == "function"){
                        dx.val(fechas.toInputDatetime(val));
                    }
                }else if(dx.attr("type") == "checkbox" || dx.attr("type") == "radio"){
                    dx.prop("checked", val);
                }else{
                    dx.val(val);
